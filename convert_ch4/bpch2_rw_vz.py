...
class bpgrid:
	...
	def __init__(self, ix, jx, lx, \
		...
		if (lonres==None):
                    ...
                    self.lonres=lonres
                    self.ix=int(360.0/lonres)
                    
                    
			
		if (latres==None):
                    self.latres=180.0/(jx-1)
		else:
                    self.latres=latres
                    self.jx=int(180.0/latres)+1

                self.mod_res=cmg.get_model_resolution(self.ix, self.jx)
		self.lats=cmg.get_model_lat(self.mod_res, self.jx)
		self.lons=cmg.get_model_lon(self.mod_res, self.ix)
	...
	def get_lat(self):
            ...
            lat=cmg.get_model_lat(self.mod_res, self.jx)
            return lat
        
           
	def get_lon(self):
            # print 'mod in lon', self.mod_res
            lon=cmg.get_model_lon(self.mod_res, self.ix)
            return lon
		...
		if (mode=='r'):
			...
			if (do_read==1):
				...
				if (stat<>0):
					...
					vtracer_id_lst,vhalfpolar_lst,vcentre180_lst,vni_lst,vnj_lst,vnl_lst,\
                                            vifirst_lst,vjfirst_lst,vlfirst_lst,vlonres_lst,vlatres_lst,\
						  vtau0_lst,vtau1_lst,vmodelname_lst,vcategory_lst,\
                                                  vunit_lst,vreserved_lst,vdata_array_lst,stat_lst, nrec = bpch2_rw_py.read_bpch2_all_record(funit)
					print 'nrec:', nrec
                                        
                                        vcategory_lst=vcategory_lst.split(';')
                                        vunit_lst=vunit_lst.split(';')
                                        vreserved_lst=vreserved_lst.split(';')
                                        vmodelname_lst=vmodelname_lst.split(';')
                                        zzx=vcategory_lst[1]
                                        ...
                                        if (nrec>0):
                                            for idx in range(nrec):
                                                vtracer_id,vhalfpolar,vcentre180,vni,vnj,vnl=vtracer_id_lst[idx],vhalfpolar_lst[idx],vcentre180_lst[idx],vni_lst[idx],vnj_lst[idx],vnl_lst[idx]
                                                
                                                vifirst,vjfirst,vlfirst,vlonres,vlatres=vifirst_lst[idx],vjfirst_lst[idx],vlfirst_lst[idx],vlonres_lst[idx],vlatres_lst[idx]
                                               
                                                vtau0,vtau1,vmodelname,vcategory=vtau0_lst[idx],vtau1_lst[idx],vmodelname_lst[idx],vcategory_lst[idx]
                                                
                                                vunit,vreserved=vunit_lst[idx],vreserved_lst[idx]
                                                
                                                vdata_array=vdata_array_lst[:,:,:,idx]
